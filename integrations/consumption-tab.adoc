...
** *Acima da capacidade máxima*: Consumo de armazenamento além do limite máximo. Essa métrica aparece somente quando o limite máximo é excedido.
+
...
Você pode visualizar a projeção de uso da capacidade para os próximos 120 dias a partir da data atual através da opção *Previsão*, disponível na visualização *Histórico de consumo* da guia *Tendência de consumo* no Console. A opção está ativada por padrão.
...
Se você selecionar um intervalo de datas que não termine na data atual, a previsão será desativada automaticamente. Para reativá-la, uma caixa de diálogo de confirmação informa que os dados da previsão só estão disponíveis quando o período selecionado termina na data atual. A confirmação redefine a visualização para o intervalo padrão dos últimos 30 dias de dados históricos mais os próximos 120 dias de previsão.
...
[TIP]
====
* No Console, você pode interagir com o gráfico de faturamento acumulado para filtrar o uso diário de dados. Para saber mais, consulte link:../integrations/consumption-tab.html#view-daily-accrued-burst-data-usage["Ver o uso diário acumulado de dados em burst"].
...
Você também pode baixar os dados em formato Excel, incluindo o status do burst (faturado, não faturado ou provisório) e a cobrança por
